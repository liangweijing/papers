...
Page({
  data: {
    ...
    tip_content: '',
    tip_btn: 0,
    showTip: wx.getStorageSync('showTip')
  },
  onLoad () {
    wx.showShareMenu({
  ...
  getList () {
    app.request('/journals/hot?offset=0&limit=300').then(res => {
      ...
      app.request('/notify').then(res => {
        this.setData({
          tip_content: res.data.tip_content,
          tip_btn: res.data.tip_btn
        });
        if (this.data.tip_btn == 1 && this.data.showTip != 'displayed') {
          this.setData({
            notifyName: 'notify'
          });
          wx.setStorageSync('showTip', 'displayed');
        }
      });
    });
  },
